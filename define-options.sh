...
[[ -z $BASH_TOYS_INTERACTIVE_FILTER ]] && export BASH_TOYS_INTERACTIVE_FILTER='peco --select-1 --initial-filter Fuzzy'
[[ -z $BASH_TOYS_DUSTBOX_DIR ]] && export BASH_TOYS_DUSTBOX_DIR="$HOME/.backup/dustbox"
[[ -z $BASH_TOYS_RESTORE_KEEP ]] && export BASH_TOYS_RESTORE_KEEP=  # Set to '1' to make --keep the default for --restore
[[ -z $BASH_TOYS_WHEN_POMODORO_TIMER_FINISHED ]] && export BASH_TOYS_WHEN_POMODORO_TIMER_FINISHED="notify 'Pomodoro Timer' 'Pomodoro complete!' $dir/assets/notification-impact.mp3"
[[ -z $BASH_TOYS_POMODORO_DEFAULT_INTERVAL ]] && export BASH_TOYS_POMODORO_DEFAULT_INTERVAL=25  # When not specified minutes for pomodoro-timer command, like `$ pomodoro-timer`
...
[[ -z $BASH_TOYS_KILL_LISTED_PROCESS_NAMES ]] && export BASH_TOYS_KILL_LISTED_PROCESS_NAMES= # Space-separated process names to kill (empty by default; e.g., 'nvim claude java npm node deno')
[[ -z $BASH_TOYS_KILL_LISTED_APPS ]] && export BASH_TOYS_KILL_LISTED_APPS= # Space-separated macOS app names to quit (empty by default; e.g., 'OpenDeck')
